...
public class RepositoryArchitectureTests
{
    ...
    public RepositoryArchitectureTests()
    {
        ...
        _applicationAssembly = typeof(ILicenseRepository).Assembly; // LicenseService.Application

        // Try to load the Persistence assembly by name - it may not be loaded in the test AppDomain yet
        try
        {
            _persistenceAssembly = Assembly.Load("LicenseService.Persistence");
        }
        catch
        {
            ...
            _persistenceAssembly = AppDomain.CurrentDomain.GetAssemblies()
                .FirstOrDefault(a => a.GetName().Name == "LicenseService.Persistence");
        }

        if (_persistenceAssembly == null)
        {
            // As a last resort, try to resolve a known type from Persistence and obtain its assembly
            var dataContextType = Type.GetType("LicenseService.Persistence.Data.DataContext, LicenseService.Persistence");
        ...
        if (_persistenceAssembly == null)
        {
            ...
            throw new InvalidOperationException("Could not load LicenseService.Persistence assembly");
        }
    ...
    private List<Type> GetDomainEntities()
    {
        ...
        var domainAssembly = AppDomain.CurrentDomain.GetAssemblies()
            .FirstOrDefault(a => a.GetName().Name == "LicenseService.Domain");
    ...
    public void Repository_Interfaces_Should_Follow_Naming_Convention()
    {
        ...
        foreach (var repoInterface in _repositoryInterfaces)
        {
            ...
            {
                failures.Add($"{repoInterface.Name} should include entity name (e.g., ILicenseRepository)");
            }
        }
